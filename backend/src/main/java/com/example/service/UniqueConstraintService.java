...
public class UniqueConstraintService {
    ...
    public void validateMovieUniqueness(Movie movie) throws ValidationException {
        ...
        boolean exists;
        if (movie.getId() != null) {
            exists = movieRepository.existsByNameAndOperatorAndDirectorExcludingId(
                    movie.getName(), operatorId, directorId, movie.getId());
        } else {
            exists = movieRepository.existsByNameAndOperatorAndDirector(
                    movie.getName(), operatorId, directorId);
        }

        if (exists) {
            throw new ValidationException(
                    "Movie with name '" + movie.getName() +
    ...
    public void validatePersonUniqueness(Person person) throws ValidationException {
        ...
        if (person.getId() != null) {
            exists = personRepository.existsByNameAndBirthdayExcludingId(
                    person.getName(), person.getBirthday(), person.getId());
        } else {
            exists = personRepository.existsByNameAndBirthday(
                    person.getName(), person.getBirthday());
        }

        if (exists) {
            throw new ValidationException(
                    "Person with name '" + person.getName() +
                            "' and birthday '" + person.getBirthday() + "' already exists.");
        }
    }
}
